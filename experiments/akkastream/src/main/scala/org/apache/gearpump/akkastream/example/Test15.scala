...
object Test15 extends AkkaApp with ArgumentsParser {
  ...
  override def main(akkaConf: Config, args: Array[String]): Unit = {
    ...
    RunnableGraph.fromGraph(GraphDSL.create() { implicit builder =>
      val A: Outlet[Int] = builder.add(Source.single(0).named("A")).out
      val B: UniformFanOutShape[Int, Int] = builder.add(Broadcast[Int](2).named("B"))
      val C: UniformFanInShape[Int, Int] = builder.add(Merge[Int](2).named("C"))
      val D: FlowShape[Int, Int] = builder.add(Flow[Int].map(_ + 1).named("D"))
      val E: UniformFanOutShape[Int, Int] = builder.add(Balance[Int](2).named("E"))
      val F: UniformFanInShape[Int, Int] = builder.add(Merge[Int](2).named("F"))
      val G: Inlet[Any] = builder.add(Sink.foreach(println).named("G")).in
