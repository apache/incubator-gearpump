...
private[cassandra] class CassandraPartitionGenerator[V, T <: Token[V]] private (
  ...
  println(s"TOTAL DATA SIZE $totalDataSize")
  ...
  private def describeRing: Seq[TokenRange] = {
    ...
    val ranges = for (tr <- metadata.getTokenRanges.toSeq) yield tokenRange(tr, metadata)
...
object CassandraPartitionGenerator {
  ...
  def apply(
    ...
    new CassandraPartitionGenerator(conn, keyspaceName, tableName, splitCount, splitSize)(tokenFactory)
  }
